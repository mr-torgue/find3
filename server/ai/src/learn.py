...
class AI(object):
    ...
    def __init__(self, family=None, path_to_data=None, config=None):
        ...
        - Max missing threshold for row filtering (supports ratio)
        - Default RSSI for missing features
        - Mean imputation (stores means for classify)
        ...
        self.config = {
            "ap_whitelist": None,
            "ap_blacklist": None,
            "max_missing": 3,          # int or ratio (0<r<1)
            "default_rssi": -100.0,
            "use_mean_imputation": True,
            "models_enabled": None
        }

        # Optional: whitelist from env
    ...
    # ---------- Support ratio/int for max_missing ----------
    ...
    def do_classification(self, index, name):
        ...
        try:
            ...
            logger.error(str(e))
            return
        predict = {i: pred for i, pred in enumerate(prediction[0])}
    ...
    def _filter_header_by_ap(self, orig_header):
        """
        Apply AP whitelist/blacklist to feature header.
    ...
    def learn(self, fname):
        ...
        # Missing-count distribution (helps choose max_missing)
        if raw_rows:
        ...
        for k in range(num_features):
            if counts[k] > 0:
                ...
                mean_per_ap[k] = default_rssi
        self.mean_per_ap = mean_per_ap

        # Filter rows by missing threshold (supports ratio)
        ...
        # Auto-relax if everything would be dropped
        ...
        for name, clf in zip(names, classifiers):
            ...
            try:
                ...
                self.logger.error("%s %s", name, str(e))
    ...
    def save(self, save_file):
        ...
        self.logger.info("SAVE_SUMMARY file=%s, features=%d, models=%d, elapsed_ms=%d",
                         ...
                         elapsed_ms)
        self._process_finished(stage="save", extra={"path": save_file, "elapsed_ms": elapsed_ms})
    ...
    def load(self, save_file=None):
        ...
        if self.header:
            self._hdr_to_idx = {h: i for i, h in enumerate(self.header[1:])}
        self._process_finished(stage="load", extra={"path": save_file, "elapsed_ms": elapsed_ms})


def do():
    # unchanged demo; requires explicit paths
    ai = AI()
    params = {'quantile': .3,
              'eps': .3,
              'damping': .9,
...
def evaluate_holdout(ai, fname):
    ...
    y_true, y_pred = [], []
    total_rows = used_rows = skipped_rows = 0
